...
start(_StartType, _StartArgs) ->
    case rts_sup:start_link() of
        {ok, Pid} ->
            ...
            ok = riak_core:register(App, [{vnode_module, rts_vnode}]),
            ok = riak_core_node_watcher:service_up(rts, self()),

            ok = riak_core:register(App, [{vnode_module, rts_entry_vnode}]),
            ok = riak_core_node_watcher:service_up(rts_entry, self()),

            ok = riak_core:register(App, [{vnode_module, rts_stat_vnode}]),
